...
class Music(QDialog, Ui_Dialog):
    ...
    def on_horizontalSlider_sliderMoved(self, position):
        ...
        pygame.mixer.music.set_pos(self.horizontalSlider.value()*self.musiclength/100)
        #sleep(2)
        pygame.mixer.music.unpause()
        print('set:',  int(pygame.mixer.music.get_pos()/1000))
        self.timer.start()
        self.flag = int(self.horizontalSlider.value() - pygame.mixer.music.get_pos()/self.musiclength/10) 
        print("拖动：",  self.flag)
    @pyqtSlot(int)
...
#        self.line += 1
        filename = QFileDialog.getOpenFileName(self, '选择mp3', './', 'MP3 文件(*.mp3)')
        if filename[0]:
    ...
    def on_listWidget_clicked(self, index):
        ...
        """
        # TODO: not implemented yet
        print(f'当前:  {self.listWidget.currentRow()}  {self.listWidget.currentItem().text()}')
